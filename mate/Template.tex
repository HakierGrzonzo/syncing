...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		...
		\draw (6,3) -- (8,7) node[anchor=north west] {$\podx$};
...
\begin{figure}[h]
	\centering
	\begin{tikzpicture}
		\draw[thick, ->] (0,3) -- (8,3) node[anchor=north west] {$x$};
		\draw (1,5) parabola bend (4,1) (7, 5) node[anchor=north west] {$\dodx$};
		\draw (1.88,3.15) -- (1.88,2.85) node[anchor = north] {$-1$};
		\draw (6.14,3.15) -- (6.14,2.85) node[anchor = north] {$1$};
		\node at (1, 3.85) {$+$};
		\node at (7, 3.85) {$+$};
		\node at (4, 2.15) {$-$};
	\end{tikzpicture}
...
\rotatebox{90}{
	...
	\end{tabularx}
	\vspace{0.5cm}
	\captionof{table}{Tabela zmienności funkcji $\fodx = x^4 - 6x^2 + 8x + 24$}
\end{minipage}}
\pagebreak
...
xtick={-2,1},
ytick={1, 24, 27},]
\clip(-4.5,-1.5) rectangle (4.5,30.5);
\draw[line width=1pt,color=qqwuqq,smooth,samples=100,domain=-4:4] plot(\x,{(\x)^(4)-6*(\x)^(2)+8*(\x)+24});
\begin{scriptsize}
\draw[color=qqwuqq] (2,24) node {$\fodx =x^4 - 6x^2 + 8x +24 $};
\draw node[anchor = north east] at (4,0){x};
\draw node[anchor = north west] at (0,30){y};
\end{scriptsize}
\end{axis}
\end{tikzpicture}
\caption{Wykres funkcji $\fodx = x^4 - 6x^2 + 8x +24$ z programu \emph{Geogebra}} 
\end{figure}
